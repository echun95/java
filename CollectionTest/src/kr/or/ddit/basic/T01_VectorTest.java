...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		//Vector는 add() 매서드를 이용하여 데이터를 추가할 수 있다.
		v1.add("aaa");
		v1.add(111);
		...
		// add(index, 데이터) -> 벡터의 index전째에 데이터를 키워 넣는다.		
		v1.add(1,"kkk");
		...
		/*				=> 자료가 삭제되면 index번째 이후의 데이터들이 앞으로 자동으로 담겨져 채워진다.
						=> 반환값 : 삭제된 데이터
		
		  remove(삭제할 데이터) => 삭제 할 데이터를 찾아 삭제한다.
		  					=> 만약 삭제할 데이터가 여러개이면 앞에서부터 삭제한다.
		  					=> 삭제 할 데이터가 정수형이거나 char형 일 경우에는
		  					=> 삭제 할 데이터를 객체로 변환해서 사용해야 한다.
		  						(아니면 인데스로 인식함)*/
		v1.remove(0);
		...
		/*제너릭 타입(generic type)
		=> Collection객체를 선언할 때 <> 안에 그 Collection이 저장할 데이터 타입을 정해주는 것을 말한다.
		=> 이러식으로 선언하게 되면 그 데이터 타입이외의 데이터를 저장할 수 없다.
	    ...
	    */
		...
		Vector<Integer> v3 = new Vector<>(); //Integer만 저장 가능
		...
		//removeAll(Collection 객체) -> 벡터의 데이터중에서 "Collection객체"가 가지고 있는 데이터 모두를 삭제한다.
		...
		/*향상된 for문
		형식 for(자료형 변수명 : 배열변수나 Collection계열 변수) {
			처리할 내용들;
			...
			}
		-> 주어진 '배열변수나 Collection계열 변수'의 데이터 개수 만큼 반복한다.
		-> 반복할 때 마다 '배열변수나 Collection계열 변수'의 데이터를 하나씩 꺼내와 '변수명'에 저장한 후 '처리할 내용들'을
		      처리한다.
		*/
		...
		//만약 제너릭을 사용하지 않은 Collection을 향상된 for문으로 처리할 경우에는 Object형으로 간주한다.
		for(Object obj : v1) {
		...
		v.trimToSize(); //벡터의 용량을 현제 벡터 사이즈로 줄인다.
		...
		v.setSize(7); //현재 용량이 설정 사이즈보다 작으면 신규용량 = 현재용량 * 2
					  //그래도 설정 사이즈 보다 작다면 신규용량 = 설정사이즈로 설정된다.
