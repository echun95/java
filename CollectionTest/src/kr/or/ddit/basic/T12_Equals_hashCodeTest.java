...
public class T12_Equals_hashCodeTest {
	...
	/*
	 * HashSet, HashMap, Hashtable과 같은 객체들을 사용할 경우, 객체가 서로 같은지를 비교하기 위해 equals()매서드와
	 * hashCode()메서드를 호출한다. 그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의 해야 한다.
	 ...
	 *    - hashCode()메서드에서 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대하여 같은 hashcode값을 만들어 낼 수 있다. 그래서 객체가 같지
	 *    않더라도 hashcode가 같을 수 있다.
	 * */
