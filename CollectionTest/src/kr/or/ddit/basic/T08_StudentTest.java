  ...
  /*문제) 학번, 이름, 국어점수, 수학점수, 영어점수, 총점, 등수를 멤버변수로 갖는 Student클래스를 만든다.
	...
	*/
	...
	public static void main(String[] args) {
		...
		memlist.add(new Student("1","홍길동",40,50,60));
		memlist.add(new Student("5","변학도",50,70,15));
		memlist.add(new Student("7","성춘향",40,20,30));
		memlist.add(new Student("3","이순신",10,50,40));
		...
		for(int i = 0; i<memlist.size()-1; i++) {			
			if(memlist.get(i).getSum() == memlist.get(i+1).getSum()) {					
				if(Integer.parseInt(memlist.get(i).getNum()) < Integer.parseInt(memlist.get(i+1).getNum())) {
