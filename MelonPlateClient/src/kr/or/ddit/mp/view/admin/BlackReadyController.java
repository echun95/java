...
public class BlackReadyController  implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			list = (ArrayList<MemberVO>) as.getReadyBlackMember();
		} catch (RemoteException e) {
			e.printStackTrace();
		}
				...
				tableview.setOnMouseClicked(new EventHandler<Event>() {
					...
					public void handle(Event event) {
						if (tableview.getSelectionModel().getSelectedItem() != null) {
							...
							//btn_black.setDisable(false);
							//btn_blackCheck.setDisable(false);
	...
	@FXML
	public void black(ActionEvent event) throws Exception {
		...
		int cnt = as.insertBlackMember(mvo);
		if (cnt > 0) {
			infoMsg("블랙 결과", cnt + "명이 블랙 되었습니다.");
			ArrayList<MemberVO> list = new ArrayList<>();
			list = (ArrayList<MemberVO>) as.getReadyBlackMember();
			allTableData.setAll(list);
			page_allMember.setPageFactory(this::createPage);
	...
	@FXML
	public void getBlack(ActionEvent event) throws Exception {
		try {
