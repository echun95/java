...
public class HolicController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			list = (ArrayList<MemberVO>) as.getAllHolicMember();
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	...
	@FXML
	public void downGrade(ActionEvent event) throws Exception {
		...
		int cnt = as.deleteHolicMember(mvo);
		if (cnt > 0) {
			infoMsg("강등 결과", cnt + "명이 강등되었습니다.");
			ArrayList<MemberVO> list = new ArrayList<>();
			list = (ArrayList<MemberVO>) as.getAllHolicMember();
			allTableData.setAll(list);
			page_allMember.setPageFactory(this::createPage);
		} else {
			errMsg("삭제 결과", "삭제된 게시글이 없습니다.");
			page_allMember.setPageFactory(this::createPage);
		}
	}
	
	@FXML
	public void holicCheck(ActionEvent event) throws Exception {
		try {
			Parent root =FXMLLoader.load(getClass().getResource("holicReady.fxml"));
