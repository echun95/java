...
class ShareData{
	public double result; //원주율 저장
	/*
	 * volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킨다.
	 * 				즉, 값이 변경되는 즉시 변수에 적용시킨다.
	 * 				다중 쓰레드에서 하나의 변수가 완벽하게 한번에 작동할수있도록 보장하는 키워드 (일종의 동기화)
	 */
	volatile public boolean isOk = false; //원주율이 계산되었는지 나타내는 변수
}
...
class CalcPIThread extends Thread{
	...
	public void run() {
		...
		for (int i = 1; i <= 1500000000; i+=2) {
			if(((i/2) % 2) == 0) {//2로 나눈 몫이 짝수이면(2로 나누었을때 나머지 0)
				sum += (1.0/i);
			}else { //2로 나눈 몫이 홀수이면
				sum -= (1.0/i);
			}
